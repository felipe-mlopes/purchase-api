import { FetchOrdersWithRejectedStatus } from "./fetch-orders-with-rejected-status";
...
describe('Fetch Orders With Rejected Status', () => {
    let inMemoryEmployeesRepository: InMemoryEmployeesRepository
    let inMemoryOrdersRepository: InMemoryOrdersRepository
    let sut: FetchOrdersWithRejectedStatus

    beforeEach(() => {
        inMemoryEmployeesRepository = new InMemoryEmployeesRepository()
        inMemoryOrdersRepository = new InMemoryOrdersRepository()
        sut = new FetchOrdersWithRejectedStatus(inMemoryOrdersRepository, inMemoryEmployeesRepository)
    })

    it('should be able to fecth orders with rejected status', async () => {
        ...
        const order01 = makeOrder({ authorId: employeeRequest.id, status: Status.REJECTED, title: 'order-01' })
        const order02 = makeOrder({ authorId: employeeRequest.id, status: Status.AUTHORIZED })
        const order03 = makeOrder({ authorId: employeeRequest.id, status: Status.REJECTED, title: 'order-03' })
