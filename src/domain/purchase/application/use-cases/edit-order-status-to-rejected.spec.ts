import { EditOrderStatusToRejectedUseCase } from "./edit-order-status-to-rejected";
...
describe('Edit Order Status to Rejected', () => {
    let inMemoryOrdersRepository: InMemoryOrdersRepository
    let sut: EditOrderStatusToRejectedUseCase

    beforeEach(() => {
        inMemoryOrdersRepository = new InMemoryOrdersRepository()
        sut = new EditOrderStatusToRejectedUseCase(inMemoryOrdersRepository)
    })

    it('should be able to edit order status to rejected', async () => {
    ...
    it('should not be able to edit order status when it is not open', async () => {
        const order = makeOrder({
            status: Status.DONE
        })
